...
module type OS = sig
  val object_to_swhid:
  string -> qualifier list -> (string -> qualifier list -> identifier) ->
  identifier option
  end

module C (OS: OS) = struct
...
let content_identifier content : Lang.identifier option =
  ...
  OS.object_to_swhid git_object [] Lang.content
...
let directory_identifier entries : Lang.identifier option =
  ...
  OS.object_to_swhid git_object [] Lang.directory
...
let release_identifier target target_type name ~author date ~message :
  ...
  OS.object_to_swhid git_object [] Lang.release
...
let revision_identifier directory parents ~author ~author_date ~committer
  ...
  OS.object_to_swhid git_object [] Lang.revision
...
let snapshot_identifier (branches : (string * (string * string) option) list) :
  ...
  OS.object_to_swhid git_object [] Lang.snapshot
  end
