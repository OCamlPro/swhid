...
let () =
  ...
  if Sys.os_type = "Win32" || Sys.os_type = "Cygwin" then ()
    ...
    List.iter
      (fun (typ, hash, expected_result) ->
        match Hash.of_string hash with
        | Error _e -> assert false
        | Ok hash ->
          let core_identifier =
            Core_identifier.mk Scheme_version.default typ hash
          in
          let identifier = mk core_identifier [] in
          let result = Swhid.Download.any identifier in
          let ok = result = expected_result in
          if not ok then begin
            Format.eprintf "test failed for identifier %a@." pp identifier;
